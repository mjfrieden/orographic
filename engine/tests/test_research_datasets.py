...
from scripts.build_research_datasets import _sentinel_outcome_rows, sentinel_evaluation_summary
...
class ResearchDatasetTests(unittest.TestCase):
    def test_ledger_rows_flatten_scores_risk_and_outcomes(self) -> None:
                    ...
                    "picks": [
                        {
                            ...
                            "sentinel": {
                                "status": "ai_success_event",
                                "event_type": "earnings",
                                "event_polarity": 0.5,
                                "shadow_multiplier": 1.04,
                                "confidence": 0.7,
                                "options_impact_label": "spot_up_iv_down",
                                "recommended_use": "tie_breaker",
                                "headlines": ["AAA beats earnings"],
                                "event_context": {"dataset_tags": "fnspid"},
                            },
        ...
        self.assertEqual(rows[0]["sentinel_status"], "ai_success_event")
        self.assertEqual(rows[0]["sentinel_options_impact_label"], "spot_up_iv_down")

        sentinel_rows = _sentinel_outcome_rows(rows)
        self.assertEqual(sentinel_rows[0]["best_fixed_exit_pnl_pct"], 0.2727)
        summary = sentinel_evaluation_summary(sentinel_rows)
        self.assertEqual(summary["rows"], 1)
        self.assertEqual(summary["by_options_impact_label"][0]["sentinel_options_impact_label"], "spot_up_iv_down")
